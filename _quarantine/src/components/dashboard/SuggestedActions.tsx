...
function getPriorityIcon(priority: string) {
  switch (priority) {
    ...
    case "Medium": return <TrendingUp className="h-4 w-4 text-[var(--warning)]" />;
...
export default function SuggestedActions({ items }: { items?: ActionItem[] }) {
  const { t } = useI18n();
  ...
  return (
    <div className="p-6 rounded-xl shadow-md border border-[var(--ring)] bg-[var(--card)]">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text)]">Suggested Actions</h3>
      ...
      <ul className="space-y-4">
        {list.map(a => (
          <li key={a.id} className="rounded-lg border border-[var(--ring)] p-4 hover:bg-[var(--neutral-soft-bg)] transition">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                {a.impact && (
                  <p className="text-xs text-[var(--text-muted)] font-medium">Impact: {a.impact}</p>
                )}
              </div>
            </div>
