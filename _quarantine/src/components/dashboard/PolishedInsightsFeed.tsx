...
interface PolishedInsightsFeedProps {
  insights: Insight[];
}
...
export default function PolishedInsightsFeed({ insights }: PolishedInsightsFeedProps) {
  const { t } = useI18n();
  ...
  return (
    <div className="p-6 rounded-xl shadow-lg border border-[var(--ring)] bg-[var(--card)]">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold text-[var(--text)]">Key Insights</h3>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {/* Show Simulate button for High severity insights */}
                  {insight.severity === "High" && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs text-[var(--text)] hover:bg-[var(--neutral-soft-bg)] border-[var(--ring)] transition"
                    >
                ...
                <h4 className="font-semibold text-[var(--text)] mb-2">{insight.title}</h4>
                <p className="text-sm text-[var(--text-muted)]">{insight.body}</p>
              </div>
              
              <div className="relative">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="absolute right-0 top-0 z-10 bg-[var(--card)] border border-[var(--ring)] rounded-lg shadow-lg p-1 min-w-[120px]">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleSimulate(insight);
